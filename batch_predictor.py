...
def _inject_css_once():
    ...
    .bp-stat-card .s-value { color:#e6edf3; font-size:1.4rem; font-weight:700; font-family:'Space Mono',monospace; }
    .bp-stat-card .s-value.blue   { color:#58a6ff; }
    .bp-stat-card .s-value.green  { color:#3fb950; }
    .bp-stat-card .s-value.amber  { color:#d29922; }
    .bp-stat-card .s-value.red    { color:#f85149; }
    .bp-stat-card .s-value.purple { color:#bc8cff; }
    ...
    .bp-schema-name { color:#e6edf3; font-weight:600; flex:1; }
    .bp-schema-note {
        font-family:'Space Mono',monospace; font-size:.78rem;
        color:#8b949e;
    }
    .bp-schema-note.ok    { color:#3fb950; }
    .bp-schema-note.warn  { color:#d29922; }
    .bp-schema-note.error { color:#f85149; }
    </style>
...
def render_batch_predictor(
    ...
    if uploaded is None:
        ...
        st.markdown("""
        <div style='text-align:center;padding:3rem 2rem;color:#30363d;
                    border:1px dashed #21262d;border-radius:10px;margin-top:1rem;'>
            <div style='font-size:2.5rem;margin-bottom:1rem;'>📦</div>
            <div style='font-size:1rem;color:#8b949e;'>Drop your new data to generate predictions</div>
            <div style='font-size:.8rem;margin-top:.5rem;font-family:Space Mono,monospace;color:#30363d'>
                .xlsx · .xls · .csv
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if not st.session_state.get("bp_run_clicked"):
        st.markdown("""
        <div style='text-align:center;padding:1.5rem;color:#30363d;font-family:Space Mono,monospace;font-size:.8rem;'>
    ...
    def _highlight_low(row):
        if row.get("flag") == "LOW_CONFIDENCE":
            return ["background-color: rgba(248,81,73,.08)"] * len(row)
        return [""] * len(row)
