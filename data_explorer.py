...
def _inject_css_once():
    ...
    .de-stat-card .s-value { color:#e6edf3; font-size:1.4rem; font-weight:700; font-family:'Space Mono',monospace; }
    .de-stat-card .s-value.blue   { color:#58a6ff; }
    .de-stat-card .s-value.green  { color:#3fb950; }
    .de-stat-card .s-value.amber  { color:#d29922; }
    .de-stat-card .s-value.red    { color:#f85149; }
    .de-stat-card .s-value.purple { color:#bc8cff; }
...
def render_data_explorer(df: pd.DataFrame = None, filename: str = None):
    ...
    if df is None:
        ...
        if uploaded is None:
            ...
            <div style='text-align:center;padding:3rem 2rem;color:#30363d;
                ...
                <div style='font-size:1rem;color:#8b949e;'>Drop a file to start exploring</div>
            </div>
            """, unsafe_allow_html=True)
            return
    ...
    def _colour_missing(val):
        if isinstance(val, (int, float)):
            if val > 30: return "color: #f85149"
            if val > 5:  return "color: #d29922"
        return ""

    st.dataframe(
        display_df.style.applymap(_colour_missing, subset=["Missing %"])
