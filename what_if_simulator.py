...
def _inject_css_once():
    ...
    .wi-section {
        ...
        border-bottom:1px solid #21262d; padding-bottom:.5rem; margin:1.8rem 0 1rem;
    }
    ...
    .wi-stat-card .s-label {
        color:#8b949e; font-size:.68rem; font-weight:600; text-transform:uppercase;
        letter-spacing:1.5px; font-family:'Space Mono',monospace; margin-bottom:.35rem;
    }
    .wi-stat-card .s-value { color:#e6edf3; font-size:1.4rem; font-weight:700; font-family:'Space Mono',monospace; }
    .wi-stat-card .s-value.blue   { color:#58a6ff; }
    .wi-stat-card .s-value.green  { color:#3fb950; }
    .wi-stat-card .s-value.amber  { color:#d29922; }
    .wi-stat-card .s-value.red    { color:#f85149; }
    .wi-stat-card .s-value.purple { color:#bc8cff; }
    ...
    .wi-hero .hero-label {
        color:#8b949e; font-size:.72rem; font-weight:700;
        text-transform:uppercase; letter-spacing:2px;
        font-family:'Space Mono',monospace; margin-bottom:.4rem;
    }
    .wi-hero .hero-value {
        color:#e6edf3; font-size:2.6rem; font-weight:800;
    ...
    .wi-hero .hero-delta.up   { background:rgba(63,185,80,.12);  border:1px solid rgba(63,185,80,.3);  color:#3fb950; }
    .wi-hero .hero-delta.down { background:rgba(248,81,73,.12);  border:1px solid rgba(248,81,73,.3);  color:#f85149; }
    .wi-hero .hero-delta.same { background:rgba(139,148,158,.12);border:1px solid rgba(139,148,158,.3);color:#8b949e; }
    .wi-hero .hero-sub {
        color:#8b949e; font-size:.9rem; margin-top:.5rem;
    }

    /* boxes */
    .wi-info {
        background:rgba(88,166,255,.07); border:1px solid rgba(88,166,255,.2);
        border-radius:8px; padding:.9rem 1.2rem; color:#8b949e; font-size:.85rem; margin:.6rem 0;
    }
    .wi-info strong { color:#58a6ff; }
    .wi-good {
        background:rgba(63,185,80,.07); border:1px solid rgba(63,185,80,.25);
        border-radius:8px; padding:.9rem 1.2rem; color:#8b949e; font-size:.85rem; margin:.4rem 0;
    }
    .wi-good strong { color:#3fb950; }
    .wi-warn {
        background:rgba(210,153,34,.07); border:1px solid rgba(210,153,34,.25);
        border-radius:8px; padding:.9rem 1.2rem; color:#8b949e; font-size:.85rem; margin:.4rem 0;
    }
    .wi-warn strong { color:#d29922; }
    ...
    .wi-scenario .s-num {
        ...
        font-size:.78rem; font-weight:700; color:#bc8cff;
        width:24px; flex-shrink:0; text-align:center;
    }
    .wi-scenario .s-name { color:#e6edf3; font-size:.92rem; flex:1; }
    .wi-scenario .s-val  {
    ...
    .wi-change .c-feat { color:#e6edf3; font-weight:600; }
    .wi-change .c-from { color:#8b949e; font-family:'Space Mono',monospace; }
    .wi-change .c-to   { color:#58a6ff; font-family:'Space Mono',monospace; font-weight:700; }
    .wi-change .c-arrow{ color:#30363d; margin:0 .4rem; }
    </style>
...
def _column_meta(X_train: pd.DataFrame, col: str) -> dict:
    ...
    if pd.api.types.is_numeric_dtype(s):
        ...
        n_unique = s.nunique()
        is_int   = pd.api.types.is_integer_dtype(s) or (s % 1 == 0).all()
        ...
        return {
            ...
            "mean":    float(s.mean()),
            "std":     float(s.std()),
            "step":    float((p99 - p1) / 100) if p99 > p1 else 0.01,
            "is_int":  is_int,
        }
    # Object / categorical
    return {
...
def render_what_if(
    ...
    with st.expander("⚙️ Simulator settings", expanded=False):
        ...
        with c1:
            top_k = st.slider(
                "Features to show as sliders", 3, min(30, len(feat_order)),
                min(10, len(feat_order)), 1, key="wi_topk",
    ...
    if st.session_state.wi_scenarios:
        ...
        for i, scn in enumerate(st.session_state.wi_scenarios):
            ...
            with cs1:
                st.markdown(f"<div style='font-family:Space Mono,monospace;color:#bc8cff;"
                            f"font-weight:700;padding-top:.6rem'>{i+1:02d}</div>",
                            unsafe_allow_html=True)
            with cs2:
                st.markdown(f"<div style='padding-top:.6rem;color:#e6edf3'>{scn['name']}</div>",
                            unsafe_allow_html=True)
            with cs3:
                st.markdown(f"<div style='padding-top:.6rem;font-family:Space Mono,monospace;"
                            f"color:#58a6ff;font-weight:700;text-align:right'>{_fmt(scn['prediction'])}</div>",
                            unsafe_allow_html=True)
            with cs4:
