...
# Persist the "user clicked Run" decision across reruns. Without this,
# clicking a download button (which always reruns the script) would cause
# run_btn = False on the next pass and the entire results pane would
# disappear — that's the "app stops working after optimization" bug.
if "aml_run_clicked" not in st.session_state:
    st.session_state.aml_run_clicked = False

# A new file or new target invalidates the previous run
current_signature = (filename, target, test_size, cv_folds, fast_mode,
                      task_override, do_tune, n_iter, cat_strategy)
if st.session_state.get("aml_signature") != current_signature:
    st.session_state.aml_run_clicked = False
    st.session_state.aml_signature   = current_signature

if st.button("🚀 Run AutoPilot", use_container_width=True, type="primary"):
    st.session_state.aml_run_clicked = True

if not st.session_state.aml_run_clicked:
    st.markdown("""
...
# Cache the training results in session_state. Streamlit reruns the entire
# script on every interaction (clicking a download button, changing a sidebar
# value); without this, the user would re-train from scratch on each click —
# or worse, the partial state would corrupt and "the app stops working".
need_to_train = (st.session_state.get("aml_results_sig") != current_signature)

if need_to_train:
    # ── Benchmark ──────────────────────────────────────────────────────────────
    st.markdown('<div class="aml-section">Benchmarking algorithms</div>', unsafe_allow_html=True)

    # Disable parallel CV on large datasets — nested parallelism (n_jobs=-1 inside
    # cross_val_score plus n_jobs=-1 inside each model) blows past the 1GB RAM
    # limit on Streamlit Cloud's free tier.
    ...
    if do_tune:
        with st.spinner(f"🔧 Tuning {best_name} with {n_iter} iterations…"):
            try:
                ...
                st.warning(f"Tuning failed, falling back to default params: {e}")
    ...
    # Cache everything in session_state so reruns don't re-train
    st.session_state.aml_results      = results
    st.session_state.aml_best_name    = best_name
    st.session_state.aml_best_model   = best_model
    st.session_state.aml_best_params  = best_params
    st.session_state.aml_results_sig  = current_signature

else:
    # Already trained for this exact configuration — pull from cache
    results     = st.session_state.aml_results
    best_name   = st.session_state.aml_best_name
    best_model  = st.session_state.aml_best_model
    best_params = st.session_state.aml_best_params

# ── Display leaderboard + best-model banner (always renders) ─────────────────
...
<div class="aml-info">
    ...
    {' &nbsp;·&nbsp; tuned' if do_tune and best_params else ''}
</div>
""", unsafe_allow_html=True)

if best_params:
    # Format numpy/python values cleanly (e.g. np.int64(200) → 200)
    def _fmt_v(v):
        if hasattr(v, "item"):
            try: return v.item()
        ...
        return v
    param_str = " &nbsp;·&nbsp; ".join(
        f"<strong>{k}</strong>={_fmt_v(v)}" for k, v in best_params.items()
    )
    st.markdown(f"<div class='aml-good'>✅ Best params found: {param_str}</div>",
                unsafe_allow_html=True)
