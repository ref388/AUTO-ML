...
def detect_task(y: pd.Series) -> str:
    ...
    if n_unique <= 15 and (n_unique / max(len(y), 1)) < 0.05:
        return "classification"
    return "regression"
...
def regressors(class_weight=None, n_jobs=-1, fast: bool = False):
    """Return dict of {name: (estimator, library_tag)}. Fast mode skips slow ones."""
    cat = {
        "Ridge":                  (Ridge(),                                              "sklearn"),
    ...
    if CATBOOST_AVAILABLE:
        cat["CatBoost"] = (CatBoostRegressor(iterations=300, learning_rate=0.1,
                                              random_state=42, verbose=False), "catboost")
    return cat
    ...
    for i, (name, (mdl, lib)) in enumerate(catalog.items()):
        try:
            ...
            results[name] = {"mean": -np.inf, "std": 0, "scores": [],
                              "lib": lib, "ok": False, "error": str(e)[:80]}
        if progress_cb:
            progress_cb((i + 1) / n, name)
    return results


def tune(X_train, y_train, best_name, task, n_iter=40, cv=5, class_weight=None):
    ...
    return rs.best_estimator_, rs.best_params_   # ← FIX: trailing underscore
...
def leaderboard_html(results: dict, task: str, best_name: str) -> str:
    ...
    if failed:
        ...
        html += f"<div class='aml-warn' style='margin-top:.5rem;'>⚠️ <strong>{len(failed)}</strong> model(s) failed: {names}</div>"
...
if not ok_results:
    st.error("All algorithms failed. Check your data.")
    st.stop()
