...
def _inject_css_once():
    ...
    .fe-stat-card .s-value { color:#e6edf3; font-size:1.4rem; font-weight:700; font-family:'Space Mono',monospace; }
    .fe-stat-card .s-value.blue   { color:#58a6ff; }
    .fe-stat-card .s-value.green  { color:#3fb950; }
    .fe-stat-card .s-value.amber  { color:#d29922; }
    .fe-stat-card .s-value.purple { color:#bc8cff; }
    ...
    .fe-recipe-num {
        ...
        color:#bc8cff; width:24px; flex-shrink:0; text-align:center;
    }
    .fe-recipe-op {
        font-family:'Space Mono',monospace;
    ...
    .fe-recipe-desc { color:#e6edf3; font-size:.88rem; flex:1; }

    /* recommendation pill */
    .fe-rec {
        display:inline-block; background:rgba(188,140,255,.1);
...
def _form_outliers(df: pd.DataFrame):
    ...
    if s.empty:
        st.markdown("<div class='fe-warn'>This column has no non-missing numeric values.</div>", unsafe_allow_html=True)
    return
    q1, q3 = s.quantile(0.25), s.quantile(0.75)
    iqr = q3 - q1
    lo, hi = q1 - 1.5 * iqr, q3 + 1.5 * iqr
    ...
    if df is None:
        ...
        if uploaded is None:
            ...
            <div style='text-align:center;padding:3rem 2rem;color:#30363d;
                ...
                <div style='font-size:1rem;color:#8b949e;'>Drop a file to start engineering features</div>
            </div>
            """, unsafe_allow_html=True)
            return
        try:
